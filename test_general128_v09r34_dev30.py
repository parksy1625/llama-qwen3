...
g=General128V09R34('http://127.0.0.1:9')
correct=routed=0
for i,(x,gold) in enumerate(zip(xs,GOLD)):
    ...
    was=g.routed-route0
    correct+=int(ok); routed+=int(bool(was))
    fam=next((z.split('=',1)[1] for z in r.trace if z.startswith('family=')),None)
    print(i,'gold',gold,'pred',pred,'ok',ok,'routed',was,'family',fam,'ops',g.math.ops-ops0,'TRACE',r.trace)
print('DEV30 correct',correct,'/30 routed',routed,'/30 ops',g.math.ops)
assert correct >= 20, (correct,routed)
assert routed >= 20, (correct,routed)
