...
from general128_v09 import General128V09
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--endpoint',required=True);ap.add_argument('--samples',type=int,default=30);ap.add_argument('--seed',type=int,default=20261905);ap.add_argument('--out',default='v09_gsm_results.json');a=ap.parse_args()
    ds=load_dataset('openai/gsm8k','main',split='test'); xs=fixed_sample(ds,a.samples,a.seed)
    g=General128V09(a.endpoint); rows=[]; base=hyb=0; t0=time.time()
    for i,x in enumerate(xs):
        ...
        rows.append({'i':i,'gold':gold,'baseline':bp,'v09':hp,'base_ok':eq(bp,gold),'v09_ok':eq(hp,gold),'calls':r.cortex_calls,'trace':r.trace})
        print(i,gold,bp,hp,base,hyb,flush=True)
    result={'n':len(xs),'baseline_correct':base,'baseline_accuracy':base/len(xs),'v09_correct':hyb,'v09_accuracy':hyb/len(xs),'delta':(hyb-base)/len(xs),'seconds':time.time()-t0,'snapshot':g.snapshot(),'rows':rows}
